...
import { gtasksUpdate } from "./commands/gtasks-update.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (cmd === "migrate") {
      ...
      if (!from)
        throw new Error("Usage: pmd migrate --from <data-dir> [--dry-run]");
      db = openDb(getDbPath());
      ...
      switch (sub) {
        case "list":
          result = taskList(subArgv, repo);
          break;
        case "get":
          result = taskGet(subArgv, repo);
          break;
        case "add":
          result = taskAdd(subArgv, repo);
          break;
        case "update":
          result = taskUpdate(subArgv, repo);
          break;
        case "complete":
          result = taskComplete(subArgv, repo);
          break;
        case "details":
          result = taskDetails(subArgv, repo);
          break;
        case "details-set":
          result = taskDetailsSet(subArgv, repo);
          break;
        case "dep-add":
          result = taskDepAdd(subArgv, repo);
          break;
        default:
          throw new Error(
            `Unknown task subcommand: ${sub}. Try: list get add update complete details details-set dep-add`,
          );
      }
      ...
      switch (sub) {
        case "list":
          result = habitList([], repo);
          break;
        default:
          throw new Error(`Unknown habit subcommand: ${sub}`);
      }
      ...
      switch (sub) {
        case "list":
          result = projectList(subArgv, repo);
          break;
        case "tasks":
          result = projectTasks(subArgv, repo);
          break;
        case "update":
          result = projectUpdate(subArgv, repo);
          break;
        default:
      ...
      // ── pmd gtasks-update ─────────────────────────────────────────────────────
    } else if (cmd === "gtasks-update") {
      const subArgv = rest.slice(rest.indexOf("gtasks-update") + 1);
      console.log(await gtasksUpdate(subArgv));
      process.exit(0);
      ...
      throw new Error(
        `Unknown command: ${cmd}. Try: task habit project max-id migrate gtasks-sync gtasks-update`,
      );
    }
